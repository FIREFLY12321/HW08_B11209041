...
// Store 類別
public class Store {
	static void checkTheSame() {
		//System.out.println("processoing check the same item");
		for(int i=0;i<cart.size();i+=3) {//合併購物車
			for(int j=i+3;j<=cart.size()-3;j+=3) {
				System.out.println(cart.get(i).getName()+" "+(cart.get(j).getName()));
				if(cart.get(i).getName().equals(cart.get(j).getName())&&
						cart.get(i+1).getName().equals(cart.get(j+1).getName())&&
						cart.get(i+2).getName().equals(cart.get(j+2).getName())
				) {
					drinkAmountList.set(i/3,drinkAmountList.get(i/3)+drinkAmountList.get(j/3));
					for (int k = 0; k < 3; k++)cart.remove(j);
					drinkAmountList.remove(j/3);
				}
			}
		}
	}
        ...
        while (true) {
            ...
            switch (select) {
                ...
                case 0:
                    int totalAmount = 0;
                    for (int i = 0; i < cart.size(); i += 3)
                        totalAmount += drinkAmountList.get(i / 3) * cart.get(i).getnum();
